...
cordic_core core (.clk(clk), .rst_n(rst_n), .load(load), .step(step), .x_init(x_init), .y_init(y_init), .z_init(z_init), .x_out(x_out), .y_out(y_out), .iter(iter), .iter_done(iter_done));
